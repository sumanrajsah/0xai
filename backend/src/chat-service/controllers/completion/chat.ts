...
import { buildAndTrimMessages, calculateCredits, deductCredits, estimateInputTokens, estimateOutputTokens, estimateToolsTokens } from './credit';
    ...
    try {
        ...
        const connectionResults = await Promise.all(
            (config?.mcp_server ?? []).map(async (endpoint: any, index: number) => {
                console.log(`🔗 [${traceId}] Connecting to MCP server ${index}`, { endpoint });

                // Add timeout for MCP connection
                const connectionPromise = McpClient(db, redis, endpoint, config.mcp_tools ?? []);
                const timeoutPromise = new Promise((_, reject) => {
                    setTimeout(() => reject(new Error(`MCP server ${index} connection timeout`)), 10000); // 10 second timeout
                });
                ...
                try {
                    ...
                    console.error(`❌ [${traceId}] MCP server ${index} connection failed`, error);
                    return { tools: [], mcpClient: null, error };
                }
            })
        );
        ...
        let customTools: any[] = [web_search];
        // if (config.tools.includes('web_search')) {
        //     customTools.push(web_search)
        // }

        const modelConfig = await getAiModels(config.model, messages, [...tools, ...customTools], config.temperature, config.top_p, config.frequency_penalty, config.presence_penalty, config.supportsMedia);
        ...
        const stream = new ReadableStream({
            async start(controller) {
                ...
                if (Object.keys(accumulatedArguments).length > 0) {
                    ...
                    // if (uid) {
                    //     await storeChatData(db, redis, chat, uid, chat_id, workspace);
                    ...
                    for (const [index, args] of Object.entries(accumulatedArguments)) {
                        ...
                        try {
                            ...
                            if (functionName === "web_search") {
                                ...
                                const searchPromise = Search(JSON.parse(args).query ? { query: JSON.parse(args).query } : { link: JSON.parse(args).link });
                                const searchTimeoutPromise = new Promise((_, reject) => {
                                    setTimeout(() => reject(new Error('Web search timeout')), 20000); // 20 second timeout
                                });
                            ...
                            if (connectionResults.length > 0) {
                                ...
                                const toolCallPromise = Promise.any(
                                ...
                                const toolTimeoutPromise = new Promise((_, reject) => {
                                    setTimeout(() => reject(new Error(`Tool call timeout for ${functionName}`)), 15000); // 15 second timeout
                                });

                                const result = await Promise.race([toolCallPromise, toolTimeoutPromise]);
...
export const Chat = async (req: Request, res: Response): Promise<void> => {
    ...
    // Set up abort handler - only abort if response hasn't completed naturally
    const abortHandler = () => {
        if (!signal.aborted) {
    ...
    try {
        ...
        const response = await generateResponse(db, redis, "user", chatMessage, message.history, message.config, message.workspace, signal, message.uid, message.chat_id);
